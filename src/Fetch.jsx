...
const UserManagement = () => {
  ...
  const gettingUsers = () => {
    fetch("https://user-dummy-api.onrender.com/api/users")
  ...
  const handleUpdateUser = () => {
    fetch(
      `https://user-dummy-api.onrender.com/api/users/${userDetails.editModal._id}`,
      {
        method: "PUT",
  ...
  const userDelete = (selectedUserID) => {
    fetch(`https://user-dummy-api.onrender.com/api/users/${selectedUserID}`, {
      method: "DELETE"
    })
      .then((response) => {
        if (response.status === 200) {
