...
const Axios = () => {
  ...
  const gettingUsers = () => {
    axios.get("https://user-dummy-api.onrender.com/api/users")
      .then((response) => {
  ...
  const handleAddUser = () => {
    axios.post("https://user-dummy-api.onrender.com/api/users", userDetails.addModal,{
      headers: {
  ...
  const handleUpdateUser = () => {
    axios.put(
      `https://user-dummy-api.onrender.com/api/users/${userDetails.editModal._id}`, userDetails.editModal,
      {
        headers: {
          "Content-Type": "application/json"
        }
  ...
  const userDelete = (selectedUserID) => {
    axios.delete(`https://user-dummy-api.onrender.com/api/users/${selectedUserID}`)
      .then((response) => {
        if (response.status === 200) {
          Swal.fire({
  ...
  // axiosing Data the from the server
  useEffect(() => {
    gettingUsers();
  }, []);
